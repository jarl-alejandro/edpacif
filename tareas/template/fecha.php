...
<div class="input-group space-padding">
 ...
 <table class="table table-bordered table-default table-striped nomargin"
  ...
  <tbody>
    <?php
    $count = 0;
    $tareas = $pdo->query("SELECT * FROM v_tarea WHERE etare_fet_etare BETWEEN '$inicio' AND '$fin'");
    ...
    <?php } if($rows["etare_est_etare"] == "asginado") {?>
      <button class="btn btn-raised center avisado button__little orden_visto"
        data-id="<?= $rows["etare_cod_etare"]; ?>">
        <i class="fa fa-check text-medium"></i>
      </button>
    <?php } if($rows["etare_est_etare"] == "revisar") {?>
