...
$equipos = $pdo->query("SELECT * FROM sgmetare WHERE etare_equ_etare='$equipo'
      AND etare_fet_etare BETWEEN '$inicio' AND '$fin'");
...
Highcharts.chart('container', {
    ...
    series: [{
        ...
        while($equipo = $equipos->fetch()) {
          $index++;
          $mesStart = date("n", strtotime($equipo["sgmetare"])) - 1;
          $dayStart = date("d", strtotime($equipo["sgmetare"]));
          $dayNameStart = date("N", strtotime($equipo["sgmetare"])) - 1;
          $yearStart = date("y", strtotime($equipo["sgmetare"]));
