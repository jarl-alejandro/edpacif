<div class="panel panel-info">
  <div class="panel-heading">
    <h3 class="panel-title text-center titulo">Registrar nueva subarea</h3>
  </div>
    ...
    <form class="form-horizontal" id="areaGeneralForm">
      ...
      <div class="col-xs-9 top-space-big">
        <div class="form-group">
          <label for="general" class="col-md-4 control-label">Area</label>
          <div class="col-md-8">
            <select class="form-control" name="general" id="general">
              <option value="">Selecione el area</option>
              <?php
              $departamento = $pdo->query("SELECT * FROM sgmearea");
              while ($row = $departamento->fetch()) {
              ?>
              <option value="<?=$row["earea_cod_earea"]?>">
                <?=$row["earea_det_earea"]?>
              </option>
              <?php } ?>
            </select>
