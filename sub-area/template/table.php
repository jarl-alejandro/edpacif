...
<table class="table table-bordered table-default table-striped nomargin"
  ...
  <tbody>
    <?php
    $id = 0;
    $departamento = $pdo->query("SELECT * FROM sgmesuba ORDER BY subare_cod_subare ASC");
    while ($rows = $departamento->fetch()) {
      $id++;
    ?>
      <tr>
        <td class="principal-cuenta"><?= $rows["subare_cod_subare"]; ?></td>
        <td class="principal-cuenta"><?= $rows["subare_det_subare"]; ?></td>
        <td class="space-around">
          <button class="btn btn-primary editar"
            data-id="<?= $rows["subare_cod_subare"]; ?>">
